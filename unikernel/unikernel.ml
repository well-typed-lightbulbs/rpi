...
module Make (Console : Mirage_console.S) (Mtime : Rpi.Mtime.S) (Pwm : Rpi.Pwm.S) =
struct
  ...
  let run console =
    let rec go queue =
      ...
      >|= R.reword_error (R.msgf "%a" Console.pp_write_error)
      >>? fun () ->
      getline console queue >>? function
      | `Close -> Lwt.return_ok ()
      | `Line line -> log console "# %S" line >>= fun () -> go queue
    in
    let queue = Ke.Rke.create ~capacity:0x1000 Bigarray.char in
    log console "Initialize the internal queue." >>= fun () -> go queue
