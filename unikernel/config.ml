...
let rpi4_clock_impl ~unix =
  ...
  let module_name = match unix with
    ...
    | false -> "Rpi.Clock.Make (Rpi.Clock)" in
  impl ~packages ~connect module_name (rpi4_mtime @-> rpi4_clock)

let rpi4_clock_impl =
  match_impl (Key.value Key.target)
  [ `RPi4, rpi4_clock_impl ~unix:false
  ; `Unix, rpi4_clock_impl ~unix:true ]
...
let rpi4_gpio_impl ~unix =
  ...
  let module_name = match unix with
    ...
    | false -> "Rpi.Gpio.Make (Rpi.Gpio)" in
  impl ~packages ~connect module_name rpi4_gpio

let rpi4_gpio_impl =
  match_impl (Key.value Key.target)
  [ `RPi4, rpi4_gpio_impl ~unix:false
  ; `Unix, rpi4_gpio_impl ~unix:true ]
...
let rpi4_pwm_impl ~unix =
  ...
  let module_name = match unix with
    ...
    | false -> "Rpi.Pwm.Make (Rpi.Pwm)" in
  impl ~packages ~connect module_name (rpi4_gpio @-> rpi4_clock @-> rpi4_mtime @-> rpi4_pwm)

let rpi4_pwm_impl =
  match_impl (Key.value Key.target)
  [ `RPi4, rpi4_pwm_impl ~unix:false
  ; `Unix, rpi4_pwm_impl ~unix:true ]
...
let () = register
  ...
  ~argv:no_argv
  ~packages:[ package "rresult" ]
  "guirlande" [ main $ default_console $ rpi4_mtime_impl $ rpi4_pwm ]
