...
module Make(B: Peripheral.Base) = struct
  ...
  let gp_set0 = Mem.(B.base + 0x1Cn)

  let gp_set1 = Mem.(B.base + 0x20n)

  let gp_clr0 = Mem.(B.base + 0x28n)

  let gp_clr1 = Mem.(B.base + 0x2Cn)

  let gp_pud = Mem.(B.base + 0x94n)

  let gp_pudclk0 = Mem.(B.base + 0x98n)

  let gp_pudclk1 = Mem.(B.base + 0x98n)
